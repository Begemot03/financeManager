...
export const useNewOperationModal = defineStore("newOperationModal", () => {
    ...
    const listOfDeposites = computed<Array<{ name: string, id: number }>>(() => depositeStore.deposites.map(deposite => (
        {
            name: deposite.name, 
            id: deposite.id 
        })
    ...
    const { handleSubmit, handleReset } = useForm({
        ...
        validationSchema: {
            ...
            depositeId (value: number) {
                if(listOfDeposites.value.find((deposite) => deposite.id === value)) return true;
